...
<style>
.sub-header { text-align: center; padding: 20px 0; background: #fff; }
.sub-header h1 { font-size: 1.4rem; color: #333; margin: 0; }
.sub-grid { display: grid; grid-template-columns: 1fr; gap: 10px; padding: 15px; }
.sub-card { background: #fff; border: 1px solid #eee; border-radius: 12px; padding: 16px 20px; text-decoration: none; color: #333; font-weight: bold; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
...
<div class="sub-grid">
  <a href="{{ '/search-50on/' | relative_url }}" class="sub-card search-50-card">
    <span>🔍 50音検索（名前から探す）</span><span>＞</span>
  </a>
